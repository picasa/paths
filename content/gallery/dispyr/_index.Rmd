...
This series is an attempt for generating etching-like drawings, by adding noise and removing data to create a more natural rendering.
...
Then, this 3D point set is processed to produce multiple lines of elevation as a function of longitude, for discrete latitude values. The intersections between lines are avoided by checking for a minimum distance between a point at a given longitude and its neighbourhood.  
Finally, the overall aesthetic is obtained by adding noise and discarding data as a function of various attributes (elevation, slope, elevation dispersion). Four methods are randomly chosen, resulting in discarding about 80 % of the information present in the initial dataset : 
